...
class Battle_Encounter():
    ...
    def card_focus(self, button: tk.Button):
            button.tkraise()
            if button.cget("image") == self.attack_picture_small.name: #changes the image depending on the card
    ...
    def card_unfocus(self, button: tk.Button):
            ...
            if button.cget("image") == self.attack_picture.name: #changes the image depending on the card
    ...
    def __init__(self, root, button_pressed, map_instance):
        ...
        #The canvas that all the images will sit on
        ...
        #Sets up the enemy health counter
        ...
        if button_pressed == 9:
            #if the button pressed is the final button make the enemy harder
            ...
            match random.randint(0, 3): #randomly chooses any enemy type
                    case 0:
                        self.Enemy_image = tk.PhotoImage(
                    ...
                    case _:
                        ...
                        self.enemy.energy = 4 #sets up the player's stats
                        self.enemy.energy_limit = 4
                        self.enemy.health = 10
        ...
        self.map_instance = map_instance #sets up a reference back to the map window
